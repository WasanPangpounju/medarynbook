      ...
      <div className="mx-auto max-w-6xl px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {shopping.map((channel) => {
            const url = channel.url && channel.url !== "#" ? channel.url : "/bookstore";
            const className =
              "bg-white border border-black/10 rounded-xl p-8 flex flex-col items-center gap-3 text-center transition-all hover:border-sage hover:-translate-y-0.5";
            const content = (
              <>
            ...
            return url.startsWith("http") ? (
              ...
              <Link key={channel.name} href={url} className={className}>
                {content}
              </Link>
            );
          })}
